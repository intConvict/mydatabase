...
rl.question('Are you sure you want to wipe this cell? (y/N) ', (answer) => {
  if (answer.toLowerCase() === 'y') {
    // User confirmed, delete the directory
    try {
      // Find table
      const filePath = path.join(packageDirectory, `${cellName}.mydb`);
      if (fs.existsSync(filePath)) {
  
        // Write an empty string to the file
        fs.writeFileSync(filePath, 'id:1');
        console.log(`Emptied cell: ${cellName}`);
      } else {
